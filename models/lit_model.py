...
class LitModel(L.LightningModule):
    ...
    def beta_scheduler(self, current_epoch, cycle_length=200):
        """cyclical beta scheduler"""
        # beta =0 for the first cycle
        if current_epoch < cycle_length:
            beta = 0
        else:
            # Calculate the cycle position
            cycle_position = (current_epoch - cycle_length) % cycle_length
            # Calculate the beta value using a sine function
            beta = (1 + (sin(2 * pi * cycle_position / cycle_length))) / 2
        return beta
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.model_type == "variational":
            ...
            loss = recon_loss + max(kl_loss, 16)
    ...
    def on_validation_epoch_end(self):
        ...
        # make a directory to save the latent space
